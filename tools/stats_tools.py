...
class StatsTools(Tools):
    ...
    def data_info(self,data):
        ...
        with st.expander('数据概览',True):
            tmp1,over_view_col1,over_view_col2,over_view_col3,tmp2 =st.columns([0.1,0.8,0.6,2,0.1])
            with over_view_col1:
                st.metric(label="数据量", value=len(data))
                st.metric(label="内存占用量", value=f"{np.round(data.memory_usage(index=True, deep=True).sum()/1028,2)} Kb")
            with over_view_col2:
            ...
            with over_view_col3:
                ...
                header=dict(values=['列名','非空数据量','类型','内存占用量'],
                            align='left'),
                cells=dict(values=[data.columns.values,data.count().values,data.dtypes.apply(lambda x:x.name).values,(np.round(data.memory_usage(index=False,deep=True)/1028,2).astype(str)+'Kb').values],
                        align='left'))
                ])
                info_table.update_layout(height=150,margin=dict(t=0, l=10, r=10, b=0))
    ...
    def col_info(self,data):
        with st.expander('列分析',True):
            ...
            if col_data.dtype.name=='object':
                ...
                col_analysis_col1.plotly_chart(px.pie(col_data.value_counts().to_frame(name='count').reset_index(),values='count',names='index'))
                col_analysis_col2.plotly_chart(px.histogram(col_data,marginal='box'),use_container_width=True)
            else:
                ...
                col_analysis_col1,col_analysis_col2 =st.columns(2)
                col_analysis_col1.plotly_chart(px.box(col_data),use_container_width=True)
                col_analysis_col2.plotly_chart(px.histogram(col_data, text_auto=True),use_container_width=True)
